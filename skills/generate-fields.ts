...
export function buildGenerateFieldsPrompt(descriptor: BoardDescriptor, request: string): string {
  ...
  return [
    ...
    'Output { "fields": [ { "key": "<snake_case>", "label": "<Human Label>", "type": "<closed type>",',
    '"enrichable": <bool>, "values": ["..."] (enum only) } ] }. Each field needs BOTH key AND label',
    '(separate) — do NOT use a single "name" property.',
    '',

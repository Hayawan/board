...
export function buildComposePrompt(description: string): string {
  return [
    ...
    '',
    'EXACT output shape (every field needs BOTH a `key` AND a `label` — they are separate):',
    '{',
    '  "name": "<Board Name>",',
    '  "view": "grid" | "list",',
    '  "ingest_mode": "url-screenshot" | "url-readable" | "manual-upload",',
    '  "enrichment_prompt": "<short instruction for the AI analyst>",',
    '  "empty_state": { "head": "<3-5 word stance headline>", "body": "<one inviting sentence in the board\'s voice>" },',
    '  "fields": [',
    '    { "key": "player_count", "label": "Player count", "type": "number", "enrichable": true },',
    '    { "key": "my_rating", "label": "My rating", "type": "number", "enrichable": false }',
    '  ]',
    '}',
    '`key` is a lowercase snake_case identifier; `label` is the human-readable title. For an',
    'enum field also include "values": ["..."]. Do NOT use a single "name" property on a field.',
    '',
    '`empty_state` is the copy a NEW, still-empty board shows on first run — make it a',
    'moment of delight in the board\'s own voice: tasteful, confident, calm, never chirpy.',
    'The `head` is a short stance (like "Nothing pinned yet."); the `body` says what this',
    'board is FOR and invites the first capture, in one sentence. No emoji, no exclamation.',
    '',

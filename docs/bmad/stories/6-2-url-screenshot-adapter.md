...
- [x] **Task 1 — Write the failing screenshot-adapter tests first (TDD)** (AC: 1, 2, 3, 5)
  - [x] Create `capture/url-screenshot.test.ts`: inject a fake launcher/browser (the AC-4 param seam); assert success → asset record (path under `screenshotsDir`, dims, hash) + `fields.title`; assert an error mid-capture still closes the browser (spy on `close`) AND propagates (does not swallow→`""`); no real Chrome.
  - [x] Run; confirm red for the right reason.
- [x] **Task 2 — Port the prototype screenshot flow into the adapter (with an injectable launcher)** (AC: 1, 3, 4)
  - [x] Create `capture/url-screenshot.ts`: a `CaptureAdapter` (Story 6.1) whose `fetch(url, ctx)` **takes the launcher via parameter/ctx** (default `launchBrowser`, Story 2.3) — this seam is net-new (the prototype hardcodes the launch at `add.ts:314-319`). Port `screenshot` (`add.ts:311-337`): launch, viewport 1440×900@1.5, `page.goto(url, {waitUntil:"networkidle2", timeout})`, settle + `dismissOverlays` (`add.ts:272-309`), `page.screenshot`, extract `body.innerText` (cap 10000) + `document.title` into `fields`.
  - [x] **Do NOT preserve the prototype's error-swallow** (`add.ts:331-333` returns `""` on failure) — propagate the error so Story 5.2 marks the item `error`. Still `close()` in `finally` (AC 5).
  - [x] Store the image under `config.screenshotsDir` (Story 2.2), compute width/height/hash, return `{ fields, assets: [{ kind:"screenshot", path, width, height, hash }] }`. The stored `asset.path` is the relative `screenshots/<id>.png` form (Story 2.2 contract).
- [x] **Task 3 — Guarantee teardown** (AC: 2)
  - [x] `browser.close()` in `finally` (the prototype does this at `add.ts:334-336`). Honor the Story 5.1 cancellation signal (AbortController) so a timeout (Story 6.5) force-closes the browser. The teardown must `await` browser-close before the worker proceeds (the 5.1↔6.5 ordering contract).
- [x] **Task 4 — Register the adapter for `ingest_mode=url-screenshot`** (AC: 1)
  - [x] Register in the Story 6.1 dispatcher. The Inspiration board's descriptor has `ingest_mode: url-screenshot` (Story 1.2).
...
- `npm test` → 230 pass / 0 fail (226 prior + 4 new screenshot tests). No pollution; no circular import (adapter.ts→url-screenshot.ts runtime, url-screenshot.ts→adapter.ts type-only).

### Completion Notes List

- ✅ All 5 ACs satisfied.
- **`createUrlScreenshotAdapter({ launch?, sleep? })`** — a `CaptureAdapter` (ingestMode `url-screenshot`). Ports the prototype's `screenshot` + `dismissOverlays`: viewport 1440×900@1.5, `goto(networkidle2, 30s)`, settle → dismiss → settle → `screenshot(clip)`, then one `page.evaluate` for `{title, text}`. Stores the PNG under `screenshotsDir` (`config.screenshotsDir` default), returns `{ fields: {title, text, url}, assets: [{kind:'screenshot', path:'screenshots/<itemId>.png', width:1440, height:900, hash}] }`.
- **Injectable launcher (AC4, the seam 6.5 needs):** `launch` defaults to Story 2.3's `launchBrowser` (lazy CHROME_PATH) but is injectable; `sleep` injectable so tests don't wait the real settle delays. Tests use a fake browser whose ops are controllable — no real Chrome.
- **Capture only, no analysis (AC3):** pulls `document.title` so a no-LLM capture still has a title (the prototype got title from analysis); design analysis is Epic 7.
- **Errors PROPAGATE (AC3):** unlike the prototype's swallow→`""`, a mid-capture throw propagates so Story 5.2 marks the item `error`. Tested (screenshot throw → rejects).
- **Teardown guaranteed (AC2/AC3):** `browser.close()` in `finally` (success AND error paths, tested via a `close` spy) + an `abort` listener force-closes the browser on a timeout (Story 6.5 signal) so a hung page op unblocks and memory is released.
- **Registered (AC1):** `registerAllCaptureAdapters` now registers it for `url-screenshot` (the Inspiration board's ingest_mode). With it registered at boot, `add-item` on an Inspiration board now enqueues a real capture job (6.1 wiring activates). Tests use an empty/injected registry so they're unaffected.

### File List

- `capture/url-screenshot.ts` (new) — `createUrlScreenshotAdapter` + ported `dismissOverlays`; injectable launch/sleep; CapturePage/CaptureBrowser/LaunchFn types.
- `capture/url-screenshot.test.ts` (new) — 4 tests (capture+asset+fields, teardown+propagate on error, ingestMode, non-URL reject).
- `capture/adapter.ts` (modified) — `registerAllCaptureAdapters` registers the screenshot adapter.
- `package.json` (modified) — appended the test to the `test` script.

### Change Log

- 2026-06-20 — Story 6.2 implemented: url-screenshot CaptureAdapter (ported, injectable launcher, errors propagate, guaranteed teardown + abort force-close), registered for url-screenshot. Status → review.
